...
def compute_on_dataset(model, data_loader, device, synchronize_gather=True, timer=None, mode=None, rel_count=None):
    model.eval()
    results_dict = {}
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        with torch.no_grad():
            ...
            if cfg.TEST.BBOX_AUG.ENABLED:
                ...
                output, rel_count= model(images.to(device), targets, mode=mode, rel_count=rel_count)
            if timer:
...
def inference(
    ...
    if load_prediction_from_cache:
        ...
        predictions = compute_on_dataset(model, data_loader, device, synchronize_gather=cfg.TEST.RELATION.SYNC_GATHER, timer=inference_timer, mode=mode,rel_count=rel_count)  #f
