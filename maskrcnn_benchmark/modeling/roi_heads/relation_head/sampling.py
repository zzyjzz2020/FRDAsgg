...
class RelationSampling(object):
    ...
    def prepare_test_pairs(self, device, proposals, mode):
        # prepare object pairs for relation prediction
        rel_pair_idxs = []
        for p in proposals:
            n = len(p)
            ############## predcl注释
            predict_logits=p.get_field("predict_logits")
            sub_dist = predict_logits.view(1, n, -1).expand(n, n, -1).unsqueeze(2)
            obj_dist = predict_logits.view(n, 1, -1).expand(n, n, -1).unsqueeze(3)
            joint_dist = torch.matmul(sub_dist,obj_dist).view(n, n)  #n*n
            ##############
            cand_matrix = torch.ones((n, n), device=device) - torch.eye(n, device=device)

            if (not self.use_gt_box) and mode=="val":   # and self.test_overlap
                cand_matrix = cand_matrix.byte() & boxlist_iou(p, p).gt(0).byte() & joint_dist.gt(500).byte()  
            else:
                cand_matrix = cand_matrix.byte()
    ...
    def detect_relsample(self, proposals, targets):
        ...
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            if self.require_overlap and (not self.use_gt_box): #f f
