...
class BiasModule(nn.Module):
    def __init__(self, cfg, statistics, penalty_type, fusion_weight):
        ...
        if 'count_bias' == self.penalty_type:
    ...
    def penalty(self, pred_dist, gt=None, obj_pair_label=None):
        ...
        if self.penalty_type == 'count_bias' or self.penalty_type == 'pair_count_bias':
...
class FrequencyBias(nn.Module):
    ...
    def __init__(self, cfg, statistics, eps=1e-3):
        ...
        # if cfg.GLOBAL_SETTING.DATASET_CHOICE == 'GQA_200':
        #     pred_dist = np.log(pred_dist / pred_dist.sum(2)[:, :, None] + eps)
        ...
        penalty_fusion_weights = 1.0
        self.bias_module = BiasModule(cfg, statistics, penalty_type=penalty_type, fusion_weight=penalty_fusion_weights)
    ...
    def index_with_rwt_labels(self, labels, gt=None):
        bias = self.index_with_labels(labels)
        bias = self.generate_bias(bias, gt=gt, obj_pair=labels)
        return bias
    def generate_bias(self, bias, gt=None, obj_pair=None):
        if True:           
            bias = self.bias_module(bias, gt=gt, obj_pair=obj_pair)
        return bias
