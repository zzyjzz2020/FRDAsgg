...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, d_inner, n_head, d_k, d_v, dropout=0.1):
        ...
        self.dec_slf_attn = MultiHeadAttention(
            n_head, d_model, d_k, d_v, dropout=dropout)
    ...
    def forward(self,enc_outputs,q,non_pad_mask,slf_attn_mask):
        # q_q=self.with_pos_embed(q, q_pos)
        # q_k=self.with_pos_embed(q, q_pos)

        dec_outputs, dec_self_attn = self.dec_slf_attn(q, q, 
                                                 q, slf_attn_mask)   # dec_outputs: [batch_size, tgt_len, d_model]
                                                                                   # dec_self_attn: [batch_size, n_heads, tgt_len, tgt_len]
        dec_outputs, dec_enc_attn = self.dec_enc_attn(q, 
                                       enc_outputs, enc_outputs, slf_attn_mask)    # dec_outputs: [batch_size, tgt_len, d_model]
                                                                                   # dec_enc_attn: [batch_size, h_heads, tgt_len, src_len]
        dec_outputs *= non_pad_mask.float()
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, encoder_feats, num_objs, query_embed):
        encoder_feats = encoder_feats.split(num_objs, dim=0)
        encoder_feats = nn.utils.rnn.pad_sequence(encoder_feats, batch_first=True)
        query_embed = query_embed.split(num_objs, dim=0)
        query_embed = nn.utils.rnn.pad_sequence(query_embed, batch_first=True)
        ...
        # -- Forward
        enc_output=encoder_feats
        # query_embed=query_embed.unsqueeze(0).repeat(enc_output.shape[0],1,1)   
        #把这个换了，就简单给个交叉注意力，说法还是添加无位置信息（看看人家怎么说的
        # dec_input = torch.zeros_like(query_embed)
        
        for dec_layer in self.layer_stack:
            dec_output, dec_slf_attn, dec_enc_attn = dec_layer(
                enc_output,query_embed,
                non_pad_mask=non_pad_mask,
                slf_attn_mask=slf_attn_mask)
        # print(query_embed[0,1,:10])
        # print(query_embed[1,:10],query_embed.requires_grad)
        # query_embed=query_embed.mean(0)
...
class TransformerContext(nn.Module):
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        self.context_edge = TransformerEncoder(self.edge_layer, self.num_head, self.k_dim, 
                                                self.v_dim, self.hidden_dim, self.inner_dim, self.dropout_rate)
        #zyj-change
        self.decoder_edge = TransformerDecoder(self.edge_layer*2, self.num_head, self.k_dim, 
                                                self.v_dim, self.hidden_dim, self.inner_dim, self.dropout_rate)
        # self.learn_query=nn.Embedding(80,512)
        # self.embed_transfer=nn.Linear(1024,512)
