...
def train(cfg, local_rank, distributed, logger):
    ...
    save_matrix = os.path.join(output_dir, "last_checkpoint")
    if os.path.exists(save_matrix):
        with open(save_matrix) as f:
            cont=f.read()
            code = cont[-11:-4]
        rel_count=torch.load(output_dir+'/'+'count_{}.pt'.format(code))
    else:
        rel_count=torch.zeros(num_relation,num_relation)
    tmp=torch.zeros(num_relation,num_relation)
    ...
    for iteration, (images, targets, _) in enumerate(train_data_loader, start_iter):
        ...
        loss_dict, rel_count = model(images, targets, rel_count=rel_count) 
        ...
        if iteration % checkpoint_period == 0:
            ...
            torch.save(rel_count, output_dir+'/'+'count_{:07d}.pt'.format(iteration))  
        if iteration == max_iter:
            checkpointer.save("model_final", **arguments)
            torch.save(rel_count, output_dir+'/'+'count_final_{:07d}.pt'.format(iteration)) 
